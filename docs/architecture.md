...
UI-поверхность (A1+, B4):
- Плитка «Агент» убрана из основной сетки Mission Control (B4). Вход через «Ещё режимы» → «Агент» (gated через `FeatureSpec` с `requires=("agent_enabled",)`) или через composition-намерения в палитре «Сменить направление»: «Собери сессию», «Пробел и практика», «Конспект+граф+квиз». При `AGENT_ENABLED=false` composition-намерения падают в тьютора.
...
пустой `evidence_chunk_id` первым валидным chunk того же документа;
  явный несуществующий chunk id **не** подменяется (gate evidence fails).
...
Выравнивание — `anchor-lis-v3.4` (`app/media_alignment.py`), детерминированное и без LLM:
...
- **локальная L1-синонимия как честный fallback** — маленький статический
  словарь учебных речевых паттернов («практическое задание» ↔ «домашнее
  задание/попробуйте сами/упражнение») включается только когда обычный
  лексический путь не дал якоря — иначе случайное слово-триггер из другого
  раздела («например» → группа «пример») портило бы уже рабочий якорь.
  Якорь обязан **заземлиться**: выигравший блок обязан содержать все токены
  хотя бы одной конкретной многословной фразы вместе («домашн»+«задан» из
  «домашнее задание»), а не любые два слова из разных фраз группы — RU-стемминг
  иногда схлопывает несвязанные слова в общий корень («практически» наречие ≈
  «практическое» прилагательное), и без совместного вхождения якорь ложится на
...
- **trust-first пропуски** — синтетические meta-разделы («Главная мысль», «Карта
  лекции», «Оглавление») и пустые structural parent-разделы не якорятся по тексту
  дочерних блоков; одинокий слабый якорь прохода без перекрёстной проверки (нет
  других якорей рядом) отбрасывается — честное отсутствие таймкода лучше
  уверенного промаха;
