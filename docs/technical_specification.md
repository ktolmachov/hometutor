...
local evidence ledger и deterministic alias/duplicate candidates. Alias-кандидаты
не мутируют graph bundle; это диагностический quality-сигнал для следующей пересборки.
...
- Выравнивание разделов ↔ таймкоды — `anchor-lis-v3.3` (`app/media_alignment.py`),
  ...
  (5) synthetic meta-разделы и пустые structural parents не получают timestamp;
