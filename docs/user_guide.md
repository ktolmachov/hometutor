...
Проект local-first: материалы, индекс и учебное состояние находятся на вашей машине. Внешний LLM/embedding endpoint используется только в соответствии с вашей `.env`-конфигурацией.
...
Если включаете облачный LLM/embedding provider, содержимое запросов может уходить внешнему провайдеру. Для local-first режима используйте локальный OpenAI-compatible endpoint и локальные embeddings.
