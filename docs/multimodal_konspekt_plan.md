...
- `app/media_alignment.py` — post-hoc выравнивание `anchor-lis-v3`: (1) семантическая
  ...
  латинских терминов конспекта в кириллицу ASR: «skills» ↔ «скиллс»); (3) slide-aware
  якорение (спикер называет «слайд N» / дословное чтение заголовка по скользящему окну ~14 c);
  (4) **per-pass хронология** (`_split_passes`) — взвешенный LIS и монотонность внутри
  «прохода» конспекта (H2-группа: слайды → ключевые темы → примеры), а не единой глобальной
  цепочкой; (5) интерполяция по номерам строк внутри прохода (confidence < 0.70);
  (6) `t_end` = начало следующего раздела прохода, у хвоста — конец смыслового блока (не
