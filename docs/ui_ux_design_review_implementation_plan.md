...
full Streamlit app e2e / pixel screenshots / SR audit / studio sign-off — **open**.
...
| **W10** | Visual regression / release gates | **partially done** | auto gates + bugfix ✓; Streamlit e2e / SR / studio open |
...
| Full Streamlit app e2e / pixel screenshots | **open** |
...
Нет `tests/e2e`; blanket overwrite `docs/screenshots/final` **запрещён**.
...
- [x] Inventory: scripts / tests / demo screenshots / absence of `tests/e2e`.
...
| `tests/e2e/` | **отсутствует** | full Streamlit e2e — **open** |
...
- [ ] Full Streamlit app screenshot/DOM regression on matrix (`tests/e2e` harness).
...
1. Mission Control cold/returning — contract/fixture partial; full-app **open**;
...
| Criterion | Status |
|---|---|
| pixel/DOM acceptance without blanket overwrite | **policy held**; full-app pixels **open** |
| no horizontal overflow | **done** on HTML fixtures + D3 mobile fix; full Streamlit **open** |
...
**закрыты**; закрытие **программы как release** требует open-пунктов выше
(не помечать W10 fully done без full Streamlit visual/a11y pass).
