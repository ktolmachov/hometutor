...
from app.ui_preferences import UI_LEVEL_KEY, UI_OVERRIDES_KEY, UI_THEME_KEY
...
def test_ui_preferences_are_exported_and_imported_with_sync_bundle(tmp_path, monkeypatch) -> None:
    ...
    try:
        ...
        user_state.set_kv(UI_THEME_KEY, "ocean")
        ...
        assert exported[UI_THEME_KEY] == "ocean"

        user_state.set_kv(UI_LEVEL_KEY, "1")
        user_state.set_kv(UI_OVERRIDES_KEY, "{}")
        user_state.set_kv(UI_THEME_KEY, "forest")
        ...
        assert user_state.get_kv(UI_THEME_KEY) == "ocean"
    finally:
        config.reset_settings_cache()
        user_state_db.reset_schema_cache_for_tests()
