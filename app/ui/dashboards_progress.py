...
from app.ui.dashboards_progress_home import (
    render_progress_home_tab_impl,
    render_progress_extended_extras,
)
...
def _render_learning_progress_tab() -> None:
    ...
    Вкладки: «Главное» (следующий шаг, heatmap, radar, геймификация) и «Расширенные» (телеметрия).
    """
    ...
    _render_panel_header(
        "Прогресс обучения",
        "Следующий шаг, heatmap, radar, геймификация (Главное) · телеметрия, сводки, графики (Расширенные)",
    )
    ...
    with tab_extended:
        ...
        render_progress_extended_extras(md)
