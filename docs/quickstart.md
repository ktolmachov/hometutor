...
9. Откройте `Живой конспект`: добавьте разделы через встроенный поиск по markdown-конспектам или из уже наполненной корзины, упорядочьте их кнопками `↑`/`↓`, прочитайте сборку во вкладке `📖 Читать`, затем сохраните рабочий конспект (дословная сшивка или LLM-синтез). Во вкладке `🌐 Дальше` проверьте актуальность по «🔗 Ссылкам из лекции» и поисковикам, скопируйте deep-study промпт для облачной модели. Если у конспекта есть валидный `.media.json` sidecar, панель покажет все видео урока, а раздел — одно или несколько видео/ссылок и безопасный timestamp action. Результат сохраняется в `data/living-konspekt/` и попадёт в поиск и граф после обновления индекса.
