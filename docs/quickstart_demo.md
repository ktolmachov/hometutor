...
Каждый сценарий здесь — это последовательность **реальных кадров** из Streamlit UI, снятых Playwright'ом в ходе demo-теста. Это доказательство, что продукт делает то, что описано в [user_guide.md](user_guide.md) и [quickstart.md](quickstart.md), — не рендеры, не мокапы.
