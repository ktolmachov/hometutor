...
if selected_view == HOME_VIEW:
    ...
    if not get_settings().agent_enabled:
        st.warning(
            "Агентный режим отключён (AGENT_ENABLED=false). "
            "Включите его в .env для доступа к учебной сессии с ИИ-агентом."
        )
        if st.button("← На главную", key="agent_disabled_back"):
