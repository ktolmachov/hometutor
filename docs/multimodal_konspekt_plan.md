...
video.mp4
  ...
  → optional --remux через ffmpeg только для браузерного .mp4
...
- `docs/adr/0002-asr-dependency-strategy.md` — `faster-whisper` optional extra (PyAV декодирует без ffmpeg; ffmpeg — только `--remux`), benchmark spike.
