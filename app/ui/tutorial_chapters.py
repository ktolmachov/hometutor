...
CHAPTERS: list[TutorialChapter] = [
    TutorialChapter(
        ...
        steps=[
            TutorialStep(
                ...
                body_ru="Пройдём путь от первого ответа до expert-режима. Начнём с быстрого ответа по вашим материалам.",
    ...
    TutorialChapter(
        id="ch5_living_konspekt",
        title_ru="Глава 5. Живой конспект",
        summary_ru="Sources → Фрагменты → Reader → Память",
        level="advanced",
        estimated_minutes=6,
        cjm_stages=["Sources", "Fragments", "Reader", "Memory"],
        steps=[
            TutorialStep(
                id="qa_to_konspekt",
                title_ru="Из ответа в конспект",
                body_ru="В источниках Быстрого ответа добавьте фрагмент в Живой конспект и проверьте видео-цитату, если у лекции есть sidecar.",
                target_view="Быстрый ответ",
                target_anchor=None,
                cta_label_ru="Дальше",
                us_refs=["LK-W7", "LK-ASK-LECTURER"],
                wow=True,
            ),
            TutorialStep(
                id="reader_and_goal",
                title_ru="Цель, чтение и свои слова",
                body_ru="Откройте Живой конспект: задайте цель сборки, прочитайте фрагменты, отметьте прогресс и запишите мысль своими словами.",
                target_view="Живой конспект",
                target_anchor=None,
                cta_label_ru="Дальше",
                us_refs=["LK-W6"],
            ),
            TutorialStep(
                id="konspekt_quiz",
                title_ru="Проверка по моей сборке",
                body_ru="Во вкладке памяти запустите quiz по текущей корзине: вопросы строятся из выбранных фрагментов, а не из нового retrieval.",
                target_view="Живой конспект",
                target_anchor=None,
                cta_label_ru="К главе 6",
                us_refs=["LK-W6", "LK-SCOPED-QUIZ"],
            ),
        ],
    ),
    TutorialChapter(
        id="ch6_course_workspace",
        title_ru="Глава 6. Курс под ключ",
