...
- 2026-07-12: полное закрытие плана "Agent as One Button" (A1: UI-дверь + префилл темы из MC; A2: read-only `/agent/runs` + `/agent/runs/{run_id}`; B1: golden расширен до 8 study_session; B2: кнопки сохранения draft-карточек; C1: история агента в Прогрессе). Обновлены: api_reference.md (новые эндпоинты), architecture.md, user_guide.md, agent_roadmap.md, docs/index.md. Добавлены targeted тесты на префилл, save cards, C1.
