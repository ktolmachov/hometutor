...
- sidecar хранит `media.video.kind` и сам video source;
...
**Готовность M0a:** 95%. Оставшийся риск — только фактическая доступность JSON Schema validator
в test dependencies; если валидатора нет, M0a сначала делает lightweight internal validation, а
JSON Schema остаётся документационным контрактом до добавления dependency.
...
- `st.video(start_time=...)` проверен вручную на локальном mp4;
...
- отсутствующий ffmpeg/ASR-extra даёт понятную ошибку;
- sha256 делает запуск идемпотентным;
- transcript оказывается в индексируемом `DATA_DIR`;
