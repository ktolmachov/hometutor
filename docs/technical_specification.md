...
| AI agent | `AGENT_ENABLED`, `AGENT_TOOL_CALL_MODE`, `AGENT_MAX_STEPS`, `AGENT_MAX_RUN_TOKENS`, `AGENT_MAX_RUN_COST_USD`, `AGENT_MAX_RUN_SECONDS` |
...
input validation
  -> guardrails
  -> if query_mode="agent" and AGENT_ENABLED=true: agent read-only loop
  -> else: classify/condense/rewrite
...
Agent Coach не имеет отдельного endpoint: он идёт через `/ask` с
`query_mode="agent"` и включается флагом `AGENT_ENABLED=true`.
...
- `app/agent/runner.py`
- `app/agent/stop_controller.py`
- `app/agent/tools_rag.py`
- `app/agent/tools_learner.py`
...
| `scenario_id` | Контракт ответа |
|---|---|
| `study_session` | диагностика, что изучать, план на 10–20 минут, проверочные вопросы, draft-карточки, следующие шаги, источники |
| `graph_gap_finder` | карта пробелов, prerequisites, почему это мешает, порядок изучения, практическая проверка, источники |
| `living_konspekt_coach` | состояние конспекта, что добавить/уточнить, что повторить, проверка понимания, draft-карточки, следующий шаг, источники |
...
`Knowledge Graph` рендерится через локальный D3 asset и companion-панель Streamlit:
выбранный концепт показывает tutor/action CTA, точные разделы документов,
local evidence ledger, deterministic alias/duplicate candidates и graph quality audit
по orphan/no-doc/no-section/relation-without-evidence/source-hygiene сигналам.
...
- Выравнивание разделов ↔ таймкоды — `anchor-lis-v3.4` (`app/media_alignment.py`),
  ...
  (3) обычный лексический матчинг раздела **приоритетнее** L1-синонимии — маленький
  статический словарь учебных паттернов («практическое задание» ↔ «домашнее
  задание/упражнение») включается только если обычного совпадения не хватило, и
  якорь обязан заземлиться на всех токенах одной конкретной фразы вместе (не
  вразнобой из разных фраз группы) — иначе RU-стемминг и случайные упоминания
  синонимов дают ложные якоря; не использует LLM;
  ...
  (5) synthetic meta-разделы, пустые structural parents и одинокие слабые якоря
  прохода без перекрёстной проверки не получают timestamp;
  (6) `t_end` = начало следующего раздела прохода, у хвоста — конец смыслового блока.
