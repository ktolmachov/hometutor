...
Проект local-first: материалы, индекс и учебное состояние находятся на вашей машине. Однако **по умолчанию** `config.env` направляет embeddings через облачный endpoint (`openrouter.ai`) — при индексации и запросах чанки документов отправляются провайдеру. Для полной приватности переопределите `EMBED_API_BASE` и `EMBED_MODEL` на локальный сервер в `.env` (см. [quickstart.md](quickstart.md)).
...
По умолчанию `config.env` использует облачный embedding endpoint. Чтобы ни один фрагмент не покидал вашу машину, переопределите `EMBED_API_BASE`, `EMBED_MODEL`, `LLM_API_BASE` и `LLM_MODEL` на локальные серверы в `.env`. При использовании облачного LLM/embedding provider содержимое запросов уходит внешнему провайдеру.
