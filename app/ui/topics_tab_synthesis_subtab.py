...
def _render_synthesis_coverage(synthesis: dict[str, Any], selected_topic: dict, iv: str | None) -> None:
    ...
    if coverage.get("total"):
        ...
        st.markdown(
            f'<span style="background:{cov_color};color:#fff;border-radius:999px;'
            f'padding:0.25rem 0.7rem;font-size:0.82rem;font-weight:700;">'
            f'Покрытие: {coverage.get("covered", 0)} из {coverage["total"]} документов '
            f'({ratio_pct}%)</span>',
    ...
    except Exception as exc:  # noqa: BLE001 - UI shows persistence failures inline.
        st.error(str(exc))
    else:
...
def _render_synthesis_exports(
    synthesis: dict[str, Any],
    selected_topic: dict,
    mode: str,
    anki_pairs: list[tuple[str, str]],
) -> None:
    ...
    with synth_action_row[0]:
        if st.button(
            "Печать/чистый вид",
            key=f"print_synthesis_{selected_topic['topic_id']}",
            width="stretch",
            type="secondary",
        ):
            _open_synthesis_print_view(synthesis, selected_topic, mode)
            st.rerun()
...
def _open_synthesis_print_view(synthesis: dict[str, Any], selected_topic: dict, mode: str) -> None:
    export_md = "".join(
        [
...
def _render_synthesis_quiz_and_sources(synthesis: dict[str, Any], selected_topic: dict) -> None:
...
def _render_empty_synthesis_hint() -> None:
    st.markdown(
        """
        <div class="callout">
            ...
            <div><strong>4.</strong> Если хотите учиться пошагово, переходите в «Программа обучения» или возвращайтесь в чат с тьютором.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_topics_synthesis_subtab(*, selected_topic: dict, iv: str | None) -> None:
    ...
    if not (isinstance(synthesis, dict) and synthesis.get("topic") == selected_topic["topic_name"]):
        _render_empty_synthesis_hint()
        return

    mode = "вся тема" if synthesis.get("selection_mode") == "topic" else "выбранные документы"
    st.caption(f"Режим synthesis: {mode}")
    _render_synthesis_coverage(synthesis, selected_topic, iv)
    ...
    _render_synthesis_exports(synthesis, selected_topic, mode, anki_pairs)
    _render_synthesis_quiz_and_sources(synthesis, selected_topic)
