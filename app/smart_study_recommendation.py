...
def smart_study_contrastive_explanation(rec: SmartStudyRecommendation) -> str:
    ...
    if nav == "sm2_tutor":
        return (
            "Темы из очереди повторений созрели по расписанию; повторение сейчас поддержит долгую память лучше, чем новые карточки."
        )
...
def smart_study_why_not_others_ru(rec: SmartStudyRecommendation) -> str:
    ...
    if nav == "flashcards_review":
        ...
        paragraph = (
            "Темы из очереди повторений созрели по расписанию: окно интервальной памяти узкое. "
