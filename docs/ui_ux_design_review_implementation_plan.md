...
full-app pixel baseline / SR audit / focus-vs-sticky / keyboard-only / studio sign-off — **open**.
...
| **W10** | Visual regression / release gates | **partially done** | auto gates + bugfix ✓; live e2e scaffold + Mission Control cold smoke ✓ (W10.F1); pixel baseline / SR / focus-vs-sticky / keyboard-only / studio open |
...
| Live Streamlit e2e scaffold (`tests/e2e`) + Mission Control cold smoke | **scaffold landed (W10.F1)**; pixel baseline/diff + остальные surfaces **open** |
| Full Streamlit app pixel screenshots / DOM regression | **open** (artifacts — inventory only) |
| Focus vs sticky / full-app keyboard / SR smoke | **open** |
...
`tests/e2e` создан в W10.F1 (external-stack); blanket overwrite
...
| `tests/e2e/` | W10.F1: live Streamlit external-stack smoke; Mission Control cold на 1366/1920/390 | **auto · scaffold landed** (opt-out `HT_SKIP_E2E_LIVE=1`); pixel baseline + остальные surfaces **open** |
...
1. Mission Control cold/returning — contract/fixture partial;
   **cold live smoke landed (W10.F1)**; returning-state + pixel baseline **open**;
...
| Criterion | Status |
|---|---|
| pixel/DOM acceptance without blanket overwrite | **policy held**; live e2e scaffold **landed (W10.F1)**; pixel baseline/diff **open** |
| no horizontal overflow | **done** on HTML fixtures + D3 mobile fix + live Mission Control smoke (W10.F1); remaining live surfaces **open** |
| focus not covered by sticky | **open** (live) |
...
harness в `tests/e2e/` (external-stack mode; см. `tests/e2e/README.md`).
...
**закрыты**; live Streamlit e2e scaffold + Mission Control cold smoke
**закрыты (W10.F1)**; закрытие **программы как release** требует оставшихся
