...
Статус: кандидат-программа к утверждению владельцем продукта. Отдельная волна
получает статус «к реализации» только после регистрации в канбане репозитория
`hometutor-studio` и сверки со связанным product-планом.
...
**Статус runtime-плана:** owner-approved к описанию в этом документе
2026-07-18; в `hometutor-studio` пока не зарегистрирован и не занимает
`ready/wip` execution slot.
...
**Registration note:** когда текущий `hometutor-studio` P0-slot освободится,
W1 должен быть перенесён в backlog как отдельный P0 corrective/product package
с узким именем уровня `interactive-quiz-assessment-integrity-v1`. До этого
runtime-план остаётся SSoT для описания проблемы, scope boundary и DoD, но не
запускает командный workflow сам по себе.
...
- [ ] Light и spatial-dark используют общие semantic tokens; full dark либо
      прошёл portal gate, либо явно оставлен rejected/deferred без изменения base.
