...
|---|---|---|
| Нет ASR-конвейера приложения | Видео остаётся «слепым» форматом | `scripts/transcribe_media.py` + optional extra `asr` (PyAV декодирует без ffmpeg). ffmpeg используется для remux + A1 audio extract (`-vn -c:a copy` → `.m4a`) + A2 basket concat. UI в Живом конспекте: `st.audio` + «Скачать выпуск (m4a)». |
...
|---|---|---|---|
| **faster-whisper (large-v3 / turbo)** | Хорошее качество на RU/EN, word/segment timestamps, VAD, Python API, удобная интеграция в `scripts/` | Нужны optional dependency, CUDA-совместимость, модельный кэш; ffmpeg только для `--remux` | Рекомендация для первого ASR-конвейера. |
...
video.mp4
  ...
  → faster-whisper large-v3/turbo (PyAV decode, CUDA/CPU, VAD, timestamps)
  → optional --remux + audio extract (A1) через ffmpeg (copy-only) для .mp4 + .m4a
     (см. `extract_audio_to_m4a` и `Run-MediaKonspektPipeline.ps1`)
     (sibling `<video>.m4a` рядом с видео; используется `extract_audio_to_m4a`)
...
Дополнительно (A2): из корзины Живого конспекта можно скачать единый «выпуск» (concat всех
доверенных аудио-фрагментов + текстовое оглавление таймкодов). Реализуется в `app/media_audio.py`
(`make_basket_audio_release`) и рендере в `living_konspekt_media.py`.
```
...
- `app/media_alignment.py` — post-hoc выравнивание `anchor-lis-v3.4`: (1) семантическая
  сегментация транскрипта (`build_semantic_blocks` — TextTiling по провалам лексической
  связности: настоящие границы тем + ключевые слова, экспорт в sidecar `semantic_blocks`);
  (2) канонизация токенов (`_tokenize_canon` — RU-стемминг словоформ + транслитерация
  латинских терминов конспекта в кириллицу ASR: «skills» ↔ «скиллс»); (3) обычный
  лексический матчинг раздела приоритетнее локальной L1-синонимии («практическое
  задание» ↔ «домашнее задание/упражнение», без LLM) — синонимия включается только
  как fallback и якорь обязан заземлиться на всех токенах одной конкретной фразы
  вместе (не вразнобой из разных фраз группы), иначе RU-стемминг схлопывает
  несвязанные слова в общий корень и даёт ложные якоря; (4) slide-aware
  якорение (спикер называет «слайд N» / дословное чтение заголовка по скользящему окну ~14 c);
  (5) **per-pass хронология** (`_split_passes`) — взвешенный LIS и монотонность внутри
  «прохода» конспекта (H2-группа: слайды → ключевые темы → примеры), а не единой глобальной
  цепочкой; (6) интерполяция по номерам строк внутри прохода (confidence < 0.70);
  (7) synthetic meta-разделы, пустые structural parents и одинокие слабые якоря прохода
  без перекрёстной проверки не получают timestamp;
  (8) `t_end` = начало следующего раздела прохода, у хвоста — конец смыслового блока (не
  конец медиа). Стабильный `section_id` (контент-хэш) — первичный ключ матчинга в UI.
  Тесты: `tests/test_media_alignment.py` (сегментация, per-pass немонотонность между
  проходами, границы блоков, инварианты `t_end`, fallback-синонимия), `tests/test_media_section_matching.py`.
...
- работает offline;
- отсутствие ASR-extra даёт понятную ошибку; отсутствие ffmpeg влияет только на `--remux`;
...
| 2. Транскрибация | Запускает одну команду | `scripts/transcribe_media.py` | ожидание → «появился текст и сегменты» | нет ASR-extra → понятная ошибка; нет ffmpeg → только `--remux` пропускается | `.txt` + `.segments.json` созданы |
...
- `docs/adr/0002-asr-dependency-strategy.md` — `faster-whisper` optional extra (PyAV декодирует без ffmpeg); ffmpeg для remux + A1/A2 audio (copy). Benchmark spike.
