...
def _render_learning_progress_tab() -> None:
    ...
    with st.container(border=True):
        ...
        st.caption(
            "Краткий чек «до/после» умного роутера после действия опирается на те же локальные очереди "
            "flashcards и SM-2 по графу, что и карточка «Умный следующий шаг» выше на этой вкладке."
        )
        ...
        with u8:
            st.metric(
                ...
                help="Интервал SM-2 > 21 день (карточка «в долгой» памяти).",
            )
        with u9:
            st.metric(
