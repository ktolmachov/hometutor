...
документ; исходный черновик `d-ai-app-data-4-noble-pine.md` (session plan)
устарел и не является источником истины.
...
- `app/orchestrator_router.py::PedagogicalRouter` — реестр псевдо-агентов
  (не в production-пути). Паттерн реестра переиспользуется в tool registry.
...
`json_object` (без строгой схемы); стриминг обходит учёт стоимости
(`stream=False` захардкожен); `HARD_TOKEN_LIMIT=20_000` (`app/llm_guards.py`)
...
Факт про транспорт: `app/provider_openai.py::OpenAI._chat` пробрасывает `**kwargs`
в `chat.completions.create`, а `from_openai_message` уже парсит `tool_calls` —
native tools API достижим без переписывания транспорта; `chat_with_resilience`
тоже форвардит kwargs (fallback-цепочка сохраняется).
...
- Флаги — только через `get_settings()` (`app/config.py`): `agent_enabled=False`,
  `agent_tool_call_mode: json|native|auto`, `agent_max_steps=6`,
  `agent_max_run_tokens`, `agent_max_run_cost_usd`, `agent_max_run_seconds`.
  Doc-sync: **каждая новая настройка одновременно добавляется в `.env.example`
  и `config.env`** (tracked defaults), иначе нарушается правило репо.
...
- UI-фича: `FeatureSpec` в `app/ui/feature_registry.py` **плюс новая ветка
  `agent_enabled` в `requirement_context_ok`** (`feature_registry.py:128`) —
  сейчас функция знает только `active_course`/`has_debug_payload`/`auth_enabled`,
  а неизвестное требование возвращает `False` (фича была бы скрыта навсегда).
...
| `learner.get_profile` | learner_model_service / `build_tutor_session_state` | read |
| `cards.get_due` | `user_state_flashcards.get_due_flashcards` | read |
| `progress.get_mastery` | тонкий read-адаптер над таблицей `quiz_mastery` (`user_state_quiz`) + `analytics_service.get_advanced_analytics` | read |
...
- `progress.get_mastery` — явного read-API «get mastery» нет; нужен тонкий
  helper в `app/user_state_quiz.py` над таблицей `quiz_mastery`.
...
3. Роль-геттеры в `app/provider.py` по образцу `_build_role_llm`:
   ...
   не проверяет).
4. Стриминг агентных шагов — осознанный non-goal (`stream=False`),
   чтобы не терять cost-tracking.
...
- Финальный ответ через `apply_output_guardrails` + конверт как в
  `_assemble_rag_result`; ветка в `query_service.answer_question`;
...
- Урок 5 (мультиагентность только при потолке, Supervisor, merge,
  sandbox/least privilege) → Wave 5/6.
