...
```powershell
cd D:\Projects\hometutor
docker compose up --build -d    # daemon mode

# Или с внешними томами:
# В docker-compose.yml заменить ./data на d:\AI\app\data и т.д.
```

Или отдельный `docker-compose.prod.yml`:
```yaml
services:
  hometutor:
    build:
      context: D:\Projects\hometutor
