...
2. На welcome выберите уровень интерфейса; оставьте «Показать подсказки
   первые 10 минут».
3. Навигация: **Главная / Учиться / Память / Библиотека** (полоса сверху);
   полный список — «Ещё · все разделы».
4. Активируйте курс → **Библиотека · Ответ** (быстрый ответ) → вопрос →
   источник → **Учиться · Тьютор** → quiz «Ответить» → **Память**
   (Мнемополис / карточки) → **Главная**.
5. Справочный multi-chapter tour **не** стартует сам — только кнопка
...
выравнивание таймкодов относятся к следующему этапу (оффлайн-пайплайн). Уже существующий sidecar должен лежать
внутри `data/`, pointer в frontmatter должен быть `DATA_DIR`-relative.

Аудио (P0 Audio Podcasts): после `transcribe_media.py` (или `Run-MediaKonspektPipeline.ps1`) рядом с `.mp4`
появляется `.m4a`. Подробно про прослушивание разделов, плейлист и скачивание выпуска — в
[user_guide.md#аудио-подкасты-audio-podcasts](user_guide.md#аудио-подкасты-audio-podcasts).
Для генерации требуется `ffmpeg` в PATH.

Главная `Mission Control` собирает resume-карточки, Smart Study Router и быстрые переходы в режимы: `Быстрый ответ`, `Тьютор`, `Quiz`, `Flashcards`, `Темы`, `Курс`, `Адаптивный план`, `Knowledge Graph`, `Живой конспект`, `История`, `Метрики`, `Объяснить файл`. Агент (при `AGENT_ENABLED=true`) доступен через «Ещё режимы» и через composition-намерения в палитре «Сменить направление».
