...
Главная `Mission Control` собирает resume-карточки, Smart Study Router и быстрые переходы в режимы: `Быстрый ответ`, `Тьютор`, `Quiz`, `Flashcards`, `Темы`, `Курс`, `Адаптивный план`, `Knowledge Graph`, `История`, `Метрики`, `Объяснить файл`.
