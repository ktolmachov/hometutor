...
|---|---|---|
| Нет ASR-конвейера приложения | Видео остаётся «слепым» форматом | `scripts/transcribe_media.py` + optional extra `asr` (PyAV декодирует без ffmpeg). ffmpeg используется для remux + A1 audio extract ( -vn -c:a copy ) + A2 concat. |
