...
**W10 (release gates / visual regression) — in progress** (static/contract gates +
bugfix; full live screenshot matrix still open).
...
W10 preflight/inventory + partial gates: 2026-07-18.
...
| **W10** | Visual regression / release gates | **in progress** | static gates + bugfix; live matrix open |
...
**Статус runtime:** **in progress** — static + pure-HTML Playwright matrix landed;
**full Streamlit app e2e / pixel screenshots / SR audit** remain open (нет
`tests/e2e`, no blanket `docs/screenshots/final` overwrite).

**Зависимости:** W1–W9 (runtime done).

#### W10 inventory (2026-07-18)

| Источник | Что есть | Gate coverage |
|---|---|---|
| `tests/test_w10_release_gates.py` | inventory constants, reduced-motion audit, AA token checks, surface presence | **auto** |
| `tests/test_w10_visual_matrix.py` | Playwright viewport matrix on flashcard / host chrome (SSR+lib+mission) / D3; 200% zoom; keyboard flip; reduced-motion | **auto** (opt-out `HT_SKIP_W10_VISUAL=1` or `HT_SKIP_KG_3D_VISUAL=1`) |
| `tests/test_ui_design_tokens.py`, `test_theme_portals_contract.py` | foundation 12/16/40/44, full dark deferred, portals | **auto** |
| `tests/test_flashcards_interactive_card.py` (+ keyboard) | flip a11y, reduced-motion 3D, 44px chips, ResizeObserver | **auto** |
| `tests/test_knowledge_graph_counters.py` | Playwright viewport matrix на production 3D HTML (overflow, CTA, layout) | **auto** (opt-out `HT_SKIP_KG_3D_VISUAL=1`) |
| W1–W9 `test_*_ui_contract.py` / surface smokes | quiz, nav, library, tutor, plan, living, onboarding | **auto** (contract, not pixel) |
| `docs/screenshots/final/scenario_*` | historical demo frames (scenario_01…38) | **inventory only** — not W10 regression baseline; no auto refresh in `scripts/` |
| `scripts/` | product/integration gates (`home_rag_*`), **no** Playwright UI screenshot runner | n/a for visual matrix |
| `tests/e2e/` | **отсутствует** | full-app Streamlit e2e — **manual / live** |

**Критические поверхности → auto vs live:**

| Поверхность | Авто (contract / partial visual) | Live / manual remains |
|---|---|---|
| Mission Control / home | progressive/nav; SSR AA + host-chrome Playwright overflow/200% zoom | cold/returning full Streamlit screenshot |
| Global nav + sidebar | `test_global_navigation`, visibility | keyboard-only smoke full chrome |
| Mnemo / 3D hall | KG 3D Playwright + D3 mobile overflow / reduced-motion | spatial-dark full interaction, SR smoke |
| Flashcards review | W4 contracts + Playwright viewport/touch/keyboard/reduced-motion | host iframe in Streamlit chrome screenshots |
| Quiz | W1 integrity contract | unanswered/submitted/result frames |
| Living Konspekt | reader smoke / layout contracts | reading-route zoom + sticky focus |
| Library | 3→2→1 + address + Playwright lib-card overflow in host fixture | catalog/transfers full app |
| Tutor Chat | W9 UI contracts + handoff reduced-motion | answer/sources/next-action frames |
...
**Write-set (W10 partial close / bugfix):**

- `app/ui_theme.css` — reduced-motion hover transforms; SSR kicker/toggle AA + ≥12px;
  `--status-warn` AA;
- `app/ui/design_tokens.py` — `status-warn` token synced with CSS;
- `app/ui/assets/knowledge_graph_d3_template.html` — reduced-motion; mobile overflow clip;
  fluid panel/tools/search; closed panel `translateX(100%)`;
- `app/ui/tutor_chat_session.py` — handoff intro reduced-motion;
- `tests/test_w10_release_gates.py` — inventory + static gates;
- `tests/test_w10_visual_matrix.py` — pure-HTML Playwright matrix;
- этот документ (W10 evidence / checklist only).

**Bugfix, найденные W10 (не reopen W1–W9):**

1. `.home-dash-card` / `.mode-card` hover `transform` не гасился reduced-motion
   (design-review residual) → host CSS.
2. D3 KG template (отдельный document) без `prefers-reduced-motion` на infinite
   pulse/decay → local media query.
3. SSR kicker/toggle: `#4a9fd4` на sky ≈ fail AA + kicker below 12px → `#1f6a9a` +
   `var(--type-meta)`.
4. Tutor Q&A handoff fade без local reduced-motion gate → media query.
5. Foundation `--status-warn` `#b9770e` ≈ 3.7:1 on white → `#92600a` (AA normal text).
6. D3 on 390×844: horizontal scrollWidth growth from off-canvas `.panel`
   (`translateX(360px)`) + non-wrapping `.tools` → `#wrap` overflow clip,
   fluid widths, `translateX(100%)`, mobile `@media (max-width:640px)`.
7. Reduced-motion previously zeroed `.panel` transform (would un-hide closed
   panel) → keep closed `translateX(100%)`, only drop transitions.
...
- reduced-motion (**static + Playwright HTML fixtures closed** for flashcard /
  host chrome / D3 / kg_3d; full Streamlit app still recommended);
- 200% zoom (**HTML host chrome fixture closed**; full Streamlit open);
- keyboard-only (**flashcard Space flip closed**; full-app open);
- screen-reader smoke audit (**open**);
- empty/loading/error/degraded/offline states (**structural hooks present**;
  visual pass open).
...
```powershell
.\.venv\Scripts\python.exe -m pytest tests/test_w10_release_gates.py tests/test_w10_visual_matrix.py tests/test_ui_design_tokens.py tests/test_theme_portals_contract.py tests/test_flashcards_interactive_card.py tests/test_tutor_chat_ui_contract.py tests/test_library_schedule_ui_contract.py -q
```
...
### 10.2 Release / visual gates (W10) — частично

- [x] Инвентаризация scripts/tests/gates и critical surfaces
      (`tests/test_w10_release_gates.py` + таблица W10 inventory).
...
- [x] AA / type floor bugfix на SSR Mission Control kicker/toggle;
      foundation + spatial-dark token contrast checks в W10 tests.
...
- [x] Mnemo 3D: existing Playwright viewport/overflow smoke
...
- [ ] **Full Streamlit app** screenshot/DOM regression on matrix (`tests/e2e`
      отсутствует; demo PNG в `docs/screenshots/final` — inventory only).
- [ ] Live pass remaining: focus vs sticky in Streamlit chrome, full-app
      keyboard-only, empty/loading/error/offline **visuals**, screen-reader smoke.
...
**W10 in progress** — static + pure-HTML Playwright gates + bugfix landed;
закрытие **программы как release** требует full Streamlit visual/a11y pass
(не помечать W10 done без неё).
