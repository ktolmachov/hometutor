...
9. Откройте `Живой конспект`: соберите рабочий конспект (дословная сшивка или LLM-синтез из выбранных разделов), проверьте актуальность по «🔗 Ссылкам из лекции» и поисковикам, скопируйте deep-study промпт для облачной модели. Если у конспекта есть валидный `.media.json` sidecar, верхняя панель покажет все видео урока, а карточка раздела покажет одно или несколько видео/ссылок и безопасный timestamp action. Кнопка «➕ Добавить крупные разделы документа» быстро расширяет корзину H2-разделами выбранной лекции. Результат сохраняется в `data/living-konspekt/` и попадёт в поиск и граф после обновления индекса.
...
Главная `Mission Control` собирает resume-карточки, Smart Study Router и быстрые переходы в режимы: `Быстрый ответ`, `Тьютор`, `Quiz`, `Flashcards`, `Темы`, `Курс`, `Адаптивный план`, `Knowledge Graph`, `Живой конспект`, `История`, `Метрики`, `Объяснить файл`.
