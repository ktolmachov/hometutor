...
| `agent` | `app/routers/agent.py` | `/agent/runs`, `/agent/runs/{run_id}` (read-only observability, A2) |
...
Agent mode создаёт только compact observability-записи `agent_runs` /
`agent_steps` в per-user `user_state.db`. Он не сохраняет карточки, quiz-result,
graph edits или workbench changes. Эти product write/HITL возможности отложены
на будущие волны.
...
- `debug.answer_path.run_id`
- `debug.agent_trace.run_id`
...
Добавлено в рамках A2 плана "Agent as One Button".

| Method | Path | Назначение |
|---|---|---|
| GET | `/agent/runs?limit=` | список последних прогонов агента (compact, без полных шагов). Защищён. |
| GET | `/agent/runs/{run_id}` | полный прогон с шагами. 404 если не найден. Защищён. |

Данные приходят из `user_state_agent_runs` (санитайзинг чувствительных полей на уровне хранения).
...
Эндпоинты зарегистрированы с `_protected_dependencies` (как другие observability роутеры).
