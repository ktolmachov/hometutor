...
def smart_study_contrastive_explanation(rec: SmartStudyRecommendation) -> str:
    ...
    if nav == "qa_continue" and ("источник" in lab_l or "свериться" in lab_l):
        return (
            "Уместнее, чем сразу интерактивный quiz или длинный чат без проверенных выдержек из индекса."
        )

    if hk == "cards_due" and nav == "safe_tutor_5min":
        return (
            "Предпочтительнее жёсткого повтора карточек сейчас: вы выбрали мягкий вход в чат после отложенного интервала."
        )
    if hk == "sm2_due" and nav == "qa_continue":
        return (
            "Лучше слепого повтора концепта в чате: сначала сверьте формулировку и источники в быстром ответе."
        )
    ...
    if hk == "adaptive_plan" and nav == "qa_continue":
        return (
            "Короткая сверка с источниками перед шагом плана надёжнее, чем прыжок сразу в чат по блоку."
        )
    ...
    if nav == "quiz_recovery_tutor":
        return (
            "Важнее очередного мини-quiz: сначала разберите ошибку с тьютором, чтобы не закрепить неверную модель."
        )
    if nav == "plan_block_tutor":
        return "Лучше хаотичного переключения режимов: шаг плана связывает день в один маршрут."
    if nav == "tutor_resume":
        return "Лучше старта quiz с нуля: сохраняете нить темы и микроконтекст прошлой сессии."
    if nav == "qa_continue" and hk == "answer_ready":
        return (
            "Лучше сразу интерактивного quiz: сначала перенесите быстрый ответ в освоение темы."
        )
...
def smart_study_why_not_others_ru(rec: SmartStudyRecommendation) -> str:
    ...
    if nav == "flashcards_review":
        ...
        paragraph = (
            "Сейчас приоритет у шага смарт-роутера без изменения самих правил. Чат тьютора, интерактивный quiz, интервальный "
            "повтор карточек и экран прогресса вторичнее, чтобы этот переход не размывал параллельными режимами. Их можно "
            "открыть отдельно, когда хотите добровольный переключатель."
        )
